...
<div class="page-container">
    ...
    <a href="javascript:;" onclick="member_add('添加用户','/admin/member/add','','500')" class="btn btn-primary radius">
    <i class="Hui-iconfont">&#xe600;</i> 添加用户</a></span> </div>
	<div class="mt-20">
	<table class="table table-border table-bordered table-hover table-bg table-sort" id="tb">
		<thead>
			<tr class="text-c">
		...
		<tbody>
			...
			<tr class="text-c">
				...
				<td>{{$value->id}}</td>
				<td><img src="{{$value->avatar}}" alt="" width="50"></td>
				<td><u style="cursor:pointer" class="text-primary" onclick="member_show('{{$value->username}}','/admin/member/show','10001','400','400')">{{$value->username}}</u></td>
				<td>@if($value->gender=="1")男@elseif($value->gender=="2")女@else保密@endif</td>
				<td>{{$value->mobile}}</td>
				<td>{{$value->email}}</td>
				<td>{{$value->created_at}}</td>
				<td>@if($value->type=="1")老师@else学生@endif</td>
				<td class="td-status">
					@if($value->status=="1")
...
<script type="text/javascript" src="/admin/webuploader-0.1.5/demo-uploader.js"></script>
<script type="text/javascript">
	...
	$('#tb').dataTable({
		...
		"order":[ [1,'asc'] ],
		"aoColumnDefs": [
			{"orderable":false,"aTargets":[0,9,10,2]}// 制定列不参与排序
		]
	});
